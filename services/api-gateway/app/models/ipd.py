...
class Ward(AuditMixin, Base):
    ...
    __tablename__ = "wards"
    __table_args__ = (Index("ix_wards_facility_name", "facility_id", "name", unique=True),)
    ...
    gender_restriction: Mapped[str | None] = mapped_column(String(10))  # male, female, any
    charge_per_day_cents: Mapped[int] = mapped_column(Integer, default=0, nullable=False)  # KES cents
...
class Admission(AuditMixin, Base):
    ...
    primary_nurse_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("staff.id"))
    ...
    admitted_from: Mapped[str | None] = mapped_column(String(30))  # opd, emergency, referral, direct
    accommodation_type: Mapped[str | None] = mapped_column(String(30))  # general, semi_private, private, icu, hdu
...
class NursingNote(AuditMixin, Base):
    ...
    __table_args__ = (Index("ix_nursing_notes_admission", "facility_id", "admission_id"),)
    ...
    shift: Mapped[str | None] = mapped_column(String(10))  # morning, afternoon, night
    severity: Mapped[str | None] = mapped_column(String(10))  # normal, warning, critical
