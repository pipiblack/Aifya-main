...
class Department(AuditMixin, Base):
    ...
    __table_args__ = (Index("ix_departments_facility_code", "facility_id", "code", unique=True),)

    code: Mapped[str] = mapped_column(String(50), nullable=False)
    name: Mapped[str] = mapped_column(String(200), nullable=False)
    description: Mapped[str | None] = mapped_column(String(500))
    department_type: Mapped[str] = mapped_column(String(50), nullable=False)  # clinical, support, admin
    parent_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("departments.id"))
    head_of_department_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True))
...
class Staff(AuditMixin, Base):
    ...
    # Identity
    keycloak_user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    ...
    license_body: Mapped[str | None] = mapped_column(String(100))  # KMPDC, NCK, PPB, KMLTTB

    # Department
    department_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("departments.id"))
    primary_department_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("departments.id"))
